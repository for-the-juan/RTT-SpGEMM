...
public:
    ...
    bool allocate(std::uint64_t count, const char *label)
    {
        ...
        if (status == cudaErrorNotSupported)
        {
            ...
            status = cudaMalloc(reinterpret_cast<void **>(&pointer_), bytes);
        }
    ...
    void reset() noexcept
    {
        ...
        {
            auto &cached = cache();
            /* A single symbolic invocation has only a small bounded number
...
__global__ void make_leaf_records_kernel(
    DmmaDeviceTiles leaf, OperandRole role, std::uint32_t parent_col_count,
    LeafRecord *records, int *error)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t leaf_row =
             static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         leaf_row < static_cast<std::uint64_t>(leaf.tile_row_count);
         leaf_row += stride)
    {
        const int begin = leaf.tile_row_ptr[leaf_row];
        const int end = leaf.tile_row_ptr[leaf_row + 1];
        if (begin < 0 || end < begin || end > leaf.num_tiles)
        {
            atomicCAS(error, 0, 1);
            continue;
        }
        int previous_col = -1;
        for (int leaf_id = begin; leaf_id < end; ++leaf_id)
        {
            const int leaf_col = leaf.tile_col_idx[leaf_id];
            if (leaf_col < 0 || leaf_col >= leaf.tile_col_count ||
                leaf_col <= previous_col)
            {
                atomicCAS(error, 0, 2);
                continue;
            }
            previous_col = leaf_col;
            const std::uint32_t row_group =
                role == OperandRole::A8x4 ? 2u : 4u;
            const std::uint32_t col_group =
                role == OperandRole::A8x4 ? 4u : 2u;
            const std::uint64_t parent_row = leaf_row / row_group;
            const std::uint64_t parent_col =
                static_cast<std::uint32_t>(leaf_col) / col_group;
            LeafRecord record{};
            record.parent_key =
                parent_row * static_cast<std::uint64_t>(parent_col_count) +
                parent_col;
            record.leaf_id = static_cast<std::uint32_t>(leaf_id);
            record.slot = static_cast<std::uint8_t>(
                (leaf_row % row_group) * col_group +
                (static_cast<std::uint32_t>(leaf_col) % col_group));
            records[leaf_id] = record;
        }
    }
                                         ...
                                         std::uint64_t count, int *heads)
{
    ...
    for (std::uint64_t i =
             static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         i < count; i += stride)
        heads[i] = i == 0 || records[i - 1].parent_key != records[i].parent_key;
}

__global__ void fill_parent_arrays_kernel(
    const LeafRecord *records, const int *parent_ids, std::uint64_t leaf_count,
    std::uint32_t parent_col_count, std::uint64_t *parent_row_ptr,
    std::uint32_t *parent_col_idx, std::uint64_t *child_ptr,
    std::uint32_t *child_leaf_ids)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t i =
             static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         i < leaf_count; i += stride)
    {
        const LeafRecord record = records[i];
        const std::uint32_t parent_id =
            static_cast<std::uint32_t>(parent_ids[i]);
        child_leaf_ids[i] = record.leaf_id;
        if (i == 0 || records[i - 1].parent_key != record.parent_key)
        {
            const std::uint64_t parent_row =
                record.parent_key / parent_col_count;
            const std::uint32_t parent_col = static_cast<std::uint32_t>(
                record.parent_key % parent_col_count);
            parent_col_idx[parent_id] = parent_col;
            child_ptr[parent_id] = i;
            atomicAdd(reinterpret_cast<unsigned long long *>(
                          parent_row_ptr + parent_row + 1),
                      1ull);
        }
    }
}

__global__ void set_child_end_kernel(std::uint64_t *child_ptr,
                                     std::uint32_t parent_count,
                                     std::uint64_t leaf_count)
{
    if (blockIdx.x == 0 && threadIdx.x == 0)
        child_ptr[parent_count] = leaf_count;
}

__global__ void build_child_masks_kernel(
    const LeafRecord *records, std::uint32_t parent_count,
    const std::uint64_t *child_ptr, std::uint8_t *child_mask, int *error)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    ...
    {
        const std::uint64_t begin = child_ptr[parent];
        const std::uint64_t end = child_ptr[parent + 1];
        ...
        {
            const int slot = records[i].slot;
            if (slot < 0 || slot >= 8 || slot <= previous_slot)
            {
                atomicCAS(error, 0, 5);
                continue;
            }
...
__global__ void candidate_bitmap_or_queue_kernel(
    DeviceIndexView a, DeviceIndexView b,
    const std::uint64_t *pair_prefix, const std::uint64_t *row_work_ptr,
    const std::uint64_t *owner_item_ptr, const WorkItem *items,
    std::uint32_t row_begin, std::uint32_t row_end,
    std::uint64_t bitmap_words_per_row, unsigned long long *ticket,
    std::uint32_t *bitmap, int *error)
{
    const int lane = threadIdx.x & 31;
    const std::uint64_t first_item = owner_item_ptr[row_begin];
    const std::uint64_t batch_item_count =
        owner_item_ptr[row_end] - first_item;
    while (true)
    {
        const std::uint64_t local_item = queue_fetch(ticket, lane);
        if (local_item >= batch_item_count)
            return;
        const WorkItem item = items[first_item + local_item];
        if (item.owner < row_begin || item.owner >= row_end)
        {
            if (lane == 0)
                atomicCAS(error, 0, 35);
            continue;
        }
        const std::uint64_t owner_work =
            row_work_ptr[item.owner + 1] - row_work_ptr[item.owner];
        ...
        const std::uint64_t first_a = a.row_ptr[item.owner];
        const std::uint64_t last_a = a.row_ptr[item.owner + 1];
        for (std::uint64_t warp_base = begin; warp_base < end;
             warp_base += 32)
        {
            const std::uint64_t local = warp_base + lane;
            const unsigned int active =
                __ballot_sync(0xffffffffu, local < end);
            if (local >= end)
                continue;
            const std::uint64_t flattened = global_base + local;
            const std::uint64_t a_id =
            ...
            if (b_id >= b.row_ptr[k + 1])
            {
                atomicCAS(error, 0, 38);
                continue;
            }
            const std::uint32_t col = b.col_idx[b_id];
            const std::uint64_t word_wide =
                static_cast<std::uint64_t>(item.owner - row_begin) *
                    bitmap_words_per_row +
                col / 32u;
            if (col >= b.parent_col_count ||
                word_wide >=
                    static_cast<std::uint64_t>(row_end - row_begin) *
                        bitmap_words_per_row ||
                word_wide > 0xffffffffull)
            {
                atomicCAS(error, 0, 39);
                continue;
            }
            const unsigned int word = static_cast<unsigned int>(word_wide);
            const unsigned int bit = 1u << (col & 31u);
            const unsigned int peers = __match_any_sync(active, word);
            const unsigned int combined = __reduce_or_sync(peers, bit);
            if (lane == __ffs(static_cast<int>(peers)) - 1)
                atomicOr(bitmap + word, combined);
        }
    }
}

__global__ void candidate_bitmap_popcount_kernel(
    const std::uint32_t *bitmap, std::uint32_t row_begin,
    std::uint32_t active_rows, std::uint64_t bitmap_words_per_row,
    std::uint64_t *row_counts)
{
    const std::uint32_t local_row = blockIdx.x;
    if (local_row >= active_rows)
        return;
    __shared__ unsigned long long sums[kBuildThreads];
    unsigned long long local_sum = 0;
    const std::uint64_t base =
        static_cast<std::uint64_t>(local_row) * bitmap_words_per_row;
    for (std::uint64_t word = threadIdx.x; word < bitmap_words_per_row;
         word += blockDim.x)
        local_sum += __popc(bitmap[base + word]);
    ...
    if (threadIdx.x == 0)
        row_counts[row_begin + local_row] = sums[0];
}

__global__ void candidate_bitmap_emit_kernel(
    const std::uint32_t *bitmap, std::uint32_t row_begin,
    std::uint32_t active_rows, std::uint64_t bitmap_words_per_row,
    std::uint32_t parent_col_count, const std::uint64_t *candidate_row_ptr,
    std::uint64_t *candidate_keys, int *error)
{
    const std::uint32_t local_row = blockIdx.x;
    if (local_row >= active_rows)
        return;
    const std::uint32_t row = row_begin + local_row;
    const std::uint64_t bitmap_base =
        static_cast<std::uint64_t>(local_row) * bitmap_words_per_row;
    __shared__ unsigned long long scan[kBuildThreads];
    __shared__ unsigned long long row_cursor;
    if (threadIdx.x == 0)
        row_cursor = 0;
    __syncthreads();

    for (std::uint64_t word_base = 0; word_base < bitmap_words_per_row;
         word_base += blockDim.x)
    {
        const std::uint64_t word_id = word_base + threadIdx.x;
        const std::uint32_t word =
            word_id < bitmap_words_per_row
                ? bitmap[bitmap_base + word_id]
                : 0u;
        const unsigned long long count = __popc(word);
        scan[threadIdx.x] = count;
        __syncthreads();
        for (int offset = 1; offset < blockDim.x; offset <<= 1)
        {
            const unsigned long long add =
                threadIdx.x >= offset ? scan[threadIdx.x - offset] : 0;
            __syncthreads();
            scan[threadIdx.x] += add;
            __syncthreads();
        }
        const unsigned long long exclusive = scan[threadIdx.x] - count;
        const unsigned long long chunk_count = scan[blockDim.x - 1];
        std::uint32_t remaining = word;
        unsigned long long rank = 0;
        while (remaining != 0)
        {
            const int bit = __ffs(static_cast<int>(remaining)) - 1;
            const std::uint64_t col = word_id * 32 + bit;
            const std::uint64_t output =
                candidate_row_ptr[row] + row_cursor + exclusive + rank;
            if (col >= parent_col_count ||
                output >= candidate_row_ptr[row + 1])
                atomicCAS(error, 0, 43);
            else
                candidate_keys[output] =
        ...
        __syncthreads();
        if (threadIdx.x == 0)
            row_cursor += chunk_count;
        __syncthreads();
    }
    if (threadIdx.x == 0 &&
        row_cursor != candidate_row_ptr[row + 1] - candidate_row_ptr[row])
        atomicCAS(error, 0, 44);
}
...
__global__ void candidate_page_hash_queue_kernel(
    DeviceIndexView a, DeviceIndexView b,
    const std::uint64_t *pair_prefix, const std::uint64_t *row_work_ptr,
    const WorkItem *items, std::uint64_t item_count,
    unsigned long long *ticket, unsigned long long *hash_keys,
    std::uint32_t hash_capacity, int *error)
{
    const int lane = threadIdx.x & 31;
    while (true)
    {
        ...
        {
            if (lane == 0)
                atomicCAS(error, 0, 61);
            continue;
        }
        const std::uint64_t end =
            item_end(item, work, kCandidateWorkQuantum);
        const std::uint64_t global_base = row_work_ptr[item.owner];
        const std::uint64_t first_a = a.row_ptr[item.owner];
        const std::uint64_t last_a = a.row_ptr[item.owner + 1];
        for (std::uint64_t warp_base = begin; warp_base < end;
        ...
        {
            const std::uint64_t local = warp_base + lane;
            bool valid = local < end;
            std::uint64_t page_key = 0;
            if (valid)
            {
                const std::uint64_t flattened = global_base + local;
                const std::uint64_t a_id = locate_a_parent(
                    pair_prefix, first_a, last_a, flattened);
                valid = a_id < last_a && pair_prefix[a_id] <= flattened &&
                        pair_prefix[a_id + 1] > flattened;
                if (valid)
                {
                    const std::uint32_t k = a.col_idx[a_id];
                    const std::uint64_t b_id =
                        b.row_ptr[k] + flattened - pair_prefix[a_id];
                    valid = b_id < b.row_ptr[k + 1];
                    if (valid)
                    {
                        const std::uint32_t col = b.col_idx[b_id];
                        valid = col < b.parent_col_count;
                        page_key =
                            (static_cast<std::uint64_t>(item.owner) << 32) |
                            (col / kCandidatePageColumns);
                    }
                }
            }
            ...
            {
                if (local < end)
                    atomicCAS(error, 0, 62);
                continue;
            }
            const unsigned int peers =
                __match_any_sync(valid_lanes, page_key);
            if (lane == __ffs(static_cast<int>(peers)) - 1 &&
                !page_hash_insert_key(hash_keys, hash_capacity, page_key))
                atomicCAS(error, 0, 63);
        }
    }
}

__global__ void mark_page_hash_slots_kernel(
    const unsigned long long *hash_keys, std::uint32_t capacity,
    std::uint32_t *slot_offsets)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t slot =
             static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         slot < capacity; slot += stride)
        slot_offsets[slot] = hash_keys[slot] != 0ull;
    if (blockIdx.x == 0 && threadIdx.x == 0)
        slot_offsets[capacity] = 0;
}

__global__ void compact_page_hash_keys_kernel(
    const unsigned long long *hash_keys, const std::uint32_t *slot_offsets,
    std::uint32_t capacity, std::uint64_t *page_keys)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t slot =
         ...
         slot < capacity; slot += stride)
    {
        const unsigned long long stored = hash_keys[slot];
        if (stored != 0ull)
            page_keys[slot_offsets[slot]] = stored - 1;
    }
}

__global__ void build_page_lookup_kernel(
    const std::uint64_t *page_keys, std::uint32_t page_count,
    unsigned long long *hash_keys, std::uint32_t *hash_values,
    std::uint32_t hash_capacity, int *error)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t page =
             static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         page < page_count; page += stride)
    {
        const std::uint64_t key = page_keys[page];
        const unsigned long long stored = key + 1;
        std::uint32_t slot =
            static_cast<std::uint32_t>(mix_hash64(key)) & (hash_capacity - 1);
        bool inserted = false;
        for (std::uint32_t probe = 0; probe < hash_capacity; ++probe)
        {
            const unsigned long long prior =
                atomicCAS(hash_keys + slot, 0ull, stored);
            if (prior == 0ull || prior == stored)
            {
                hash_values[slot] = static_cast<std::uint32_t>(page);
                inserted = true;
                break;
            }
        ...
        if (!inserted)
            atomicCAS(error, 0, 64);
    }
}

__global__ void candidate_page_bitmap_queue_kernel(
    DeviceIndexView a, DeviceIndexView b,
    const std::uint64_t *pair_prefix, const std::uint64_t *row_work_ptr,
    const WorkItem *items, std::uint64_t item_count,
    unsigned long long *ticket, const unsigned long long *hash_keys,
    const std::uint32_t *hash_values, std::uint32_t hash_capacity,
    std::uint32_t page_count, std::uint32_t *page_bitmaps, int *error)
{
    const int lane = threadIdx.x & 31;
    while (true)
    {
             ...
             warp_base += 32)
        {
            ...
            {
                if (local < end)
                    atomicCAS(error, 0, 67);
                continue;
            }
            const unsigned int page_peers =
                __match_any_sync(valid_lanes, page_key);
            const int page_leader = __ffs(static_cast<int>(page_peers)) - 1;
            std::uint32_t page_id = 0xffffffffu;
            if (lane == page_leader)
                page_id = page_hash_lookup(hash_keys, hash_values,
                                           hash_capacity, page_key);
            page_id = __shfl_sync(page_peers, page_id, page_leader);
            const bool found = page_id < page_count;
            const unsigned int found_lanes =
                __ballot_sync(valid_lanes, found);
            if (!found)
            {
                atomicCAS(error, 0, 68);
                continue;
            }
            const std::uint32_t within = col % kCandidatePageColumns;
            const std::uint32_t bitmap_word =
                page_id * kCandidatePageWords + within / 32;
            const std::uint32_t bit = 1u << (within & 31u);
            const unsigned int word_peers =
                __match_any_sync(found_lanes, bitmap_word);
            const unsigned int combined = __reduce_or_sync(word_peers, bit);
            if (lane == __ffs(static_cast<int>(word_peers)) - 1)
                atomicOr(page_bitmaps + bitmap_word, combined);
        }
    }
}

__global__ void count_page_rows_kernel(const std::uint64_t *page_keys,
                                       std::uint32_t page_count,
                                       std::uint32_t parent_row_count,
                                       std::uint64_t *page_row_ptr, int *error)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t page =
    ...
    {
        const std::uint32_t row =
            static_cast<std::uint32_t>(page_keys[page] >> 32);
        if (row >= parent_row_count)
            atomicCAS(error, 0, 69);
        else
            atomicAdd(reinterpret_cast<unsigned long long *>(page_row_ptr +
                                                              row + 1),
                      1ull);
    }
}

__global__ void popcount_page_bitmaps_kernel(
    const std::uint32_t *page_bitmaps, std::uint32_t page_count,
    std::uint32_t *page_candidate_counts)
{
    const std::uint32_t page = blockIdx.x;
    const int lane = threadIdx.x;
    if (page >= page_count || lane >= kCandidatePageWords)
        return;
    unsigned int count =
        __popc(page_bitmaps[page * kCandidatePageWords + lane]);
    count = __reduce_add_sync(0xffffffffu, count);
    if (lane == 0)
        page_candidate_counts[page] = count;
}

__global__ void page_row_candidate_offsets_kernel(
    std::uint32_t parent_row_count, const std::uint64_t *page_row_ptr,
    const std::uint32_t *page_candidate_counts,
    std::uint64_t *page_candidate_local_offsets,
    std::uint64_t *candidate_row_counts)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t row =
         ...
         row < parent_row_count; row += stride)
    {
        ...
        for (std::uint64_t page = page_row_ptr[row];
             page < page_row_ptr[row + 1]; ++page)
        {
            page_candidate_local_offsets[page] = count;
            count += page_candidate_counts[page];
        }
        candidate_row_counts[row] = count;
    }
}

__global__ void emit_page_candidates_kernel(
    const std::uint64_t *page_keys, const std::uint32_t *page_bitmaps,
    std::uint32_t page_count, std::uint32_t parent_col_count,
    const std::uint64_t *page_candidate_local_offsets,
    const std::uint64_t *candidate_row_ptr, std::uint64_t *candidate_keys,
    int *error)
{
    const std::uint32_t page = blockIdx.x;
    ...
    if (page >= page_count || lane >= kCandidatePageWords)
        return;
    const std::uint64_t page_key = page_keys[page];
    const std::uint32_t row = static_cast<std::uint32_t>(page_key >> 32);
    const std::uint32_t page_col = static_cast<std::uint32_t>(page_key);
    const std::uint32_t word =
        page_bitmaps[page * kCandidatePageWords + lane];
    const unsigned int count = __popc(word);
...
#pragma unroll
    ...
    const std::uint64_t output_base =
        candidate_row_ptr[row] + page_candidate_local_offsets[page] +
        inclusive - count;
    std::uint32_t remaining = word;
    std::uint64_t rank = 0;
    while (remaining != 0)
    {
        ...
        const std::uint64_t col =
            static_cast<std::uint64_t>(page_col) * kCandidatePageColumns +
            lane * 32 + bit;
        const std::uint64_t output = output_base + rank;
        if (col >= parent_col_count || output >= candidate_row_ptr[row + 1])
            atomicCAS(error, 0, 70);
        else
            candidate_keys[output] =
                (static_cast<std::uint64_t>(row) << 32) | col;
        remaining &= remaining - 1;
...
__global__ void exact_owner_work_kernel(
    DeviceIndexView a, DeviceIndexView b, const std::uint64_t *candidate_keys,
    std::uint32_t candidate_count, std::uint64_t *owner_work, int *error)
{
    const std::uint64_t stride =
        static_cast<std::uint64_t>(gridDim.x) * blockDim.x;
    for (std::uint64_t owner =
             static_cast<std::uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         owner < candidate_count; owner += stride)
    {
        const std::uint64_t key = candidate_keys[owner];
        ...
        {
            owner_work[owner] = 0;
            atomicCAS(error, 0, 40);
            continue;
        }
        owner_work[owner] = (a.row_ptr[row + 1] - a.row_ptr[row]) +
                            (b.col_ptr[col + 1] - b.col_ptr[col]);
    }
}
...
__global__ void exact_mask_queue_kernel(
    DeviceIndexView a, DeviceIndexView b,
    const std::uint64_t *candidate_keys,
    std::uint32_t candidate_count, const std::uint64_t *owner_work,
    const WorkItem *items,
    std::uint64_t item_count, unsigned long long *ticket,
    ...
    unsigned long long *matched_k_parents, int *error)
{
    ...
    while (true)
    {
        const std::uint64_t item_id = queue_fetch_group(group, ticket);
        if (item_id >= item_count)
            return;
        const WorkItem item = items[item_id];
        const std::uint64_t diagonal_begin =
            item_begin(item, kExactWorkQuantum);
        if (item.owner >= candidate_count)
        {
            if (lane == 0)
                atomicCAS(error, 0, 41);
            continue;
        }
        const std::uint64_t work = owner_work[item.owner];
        if (diagonal_begin >= work)
        {
            if (lane == 0)
                atomicCAS(error, 0, 41);
            continue;
        ...
        const std::uint64_t key = candidate_keys[item.owner];
        const std::uint32_t row = static_cast<std::uint32_t>(key >> 32);
        const std::uint32_t col = static_cast<std::uint32_t>(key);
        const std::uint64_t a_begin = a.row_ptr[row];
        const std::uint64_t a_size = a.row_ptr[row + 1] - a_begin;
        const std::uint64_t b_begin = b.col_ptr[col];
        const std::uint64_t b_size = b.col_ptr[col + 1] - b_begin;
        const std::uint64_t diagonal_end =
            item_end(item, work, kExactWorkQuantum);

        std::uint64_t packed = 0;
        if (lane == 0)
            packed = merge_co_rank(diagonal_begin, a, b, a_begin, a_size,
                                   b_begin, b_size);
        packed = group.shfl(packed, 0);
        if (packed == ~std::uint64_t{0})
        {
            if (lane == 0)
                atomicCAS(error, 0, 42);
            continue;
        }
        std::uint64_t a_local = packed >> 32;
        std::uint64_t b_local = packed & 0xffffffffull;
        std::uint16_t local_boolean_row = 0;
        unsigned long long local_matches = 0;

        for (std::uint64_t diagonal = diagonal_begin;
             diagonal < diagonal_end; ++diagonal)
        {
            int a_parent_id = -1;
            int b_parent_id = -1;
            if (lane == 0)
            {
                const bool take_a =
                    a_local < a_size &&
                    (b_local == b_size ||
                     a.col_idx[a_begin + a_local] <=
                         b.row_idx[b_begin + b_local]);
                if (take_a)
                    ++a_local;
                else
                {
                    const std::uint32_t k = b.row_idx[b_begin + b_local];
                    if (a_local > 0 &&
                        a.col_idx[a_begin + a_local - 1] == k)
                    {
                        a_parent_id =
                            static_cast<int>(a_begin + a_local - 1);
                        b_parent_id = static_cast<int>(
                            b.csc_parent_ids[b_begin + b_local]);
                    }
                    ++b_local;
                }
            }
        ...
        unsigned long long q00 =
            lane < 8
                ? static_cast<unsigned long long>(local_boolean_row & 0xffu)
                      << (native_row * 8)
                : 0ull;
        unsigned long long q01 =
            lane < 8
                ? static_cast<unsigned long long>(local_boolean_row >> 8)
                      << (native_row * 8)
                : 0ull;
        ...
        {
            const std::uint64_t output =
                static_cast<std::uint64_t>(item.owner) * 4;
            if (q00 != 0)
                atomicOr(quadrant_masks + output, q00);
            if (q01 != 0)
                atomicOr(quadrant_masks + output + 1, q01);
            if (q10 != 0)
                atomicOr(quadrant_masks + output + 2, q10);
            if (q11 != 0)
                atomicOr(quadrant_masks + output + 3, q11);
            if (local_matches != 0)
                atomicAdd(matched_k_parents + item.owner, local_matches);
        }
    }
}
...
inline bool build_candidates_row_batch_reference(
    ...
    CandidateState *output, SymbolicMetrics *metrics = nullptr)
{
    ...
    if (a.role != OperandRole::A8x4 || b.role != OperandRole::B4x8 ||
        a.parent_col_count != b.parent_row_count || a.row_ptr == nullptr ||
        b.row_ptr == nullptr || (a.parent_count > 0 && a.col_idx == nullptr) ||
        (b.parent_count > 0 && b.col_idx == nullptr))
    {
        std::fprintf(stderr, "Invalid Super16 candidate operands.\n");
        return false;
    }

    CandidateState result;
    DeviceBuffer<int> error;
    if (!error.allocate(1, "allocate Super16 candidate error") ||
        !detail::cuda_ok(cudaMemsetAsync(error.get(), 0, sizeof(int), stream),
                         "clear Super16 candidate error") ||
        !result.a_parent_pair_prefix.allocate(
            static_cast<std::uint64_t>(a.parent_count) + 1,
            "allocate Super16 A-parent pair prefix") ||
        !result.row_work_ptr.allocate(
            static_cast<std::uint64_t>(a.parent_row_count) + 1,
            "allocate Super16 row work pointer"))
        return false;

    const unsigned int parent_blocks = detail::build_blocks(a.parent_count);
    if (parent_blocks != 0)
        detail::parent_pair_degrees_kernel
            <<<parent_blocks, kBuildThreads, 0, stream>>>(
                a, b, result.a_parent_pair_prefix.get(), error.get());
    else if (!detail::cuda_ok(cudaMemsetAsync(
                                  result.a_parent_pair_prefix.get(), 0,
                                  sizeof(std::uint64_t), stream),
                              "initialize empty Super16 pair prefix"))
        return false;
    if (!detail::cuda_ok(cudaGetLastError(),
                         "launch Super16 pair degree build"))
        return false;
    try
    {
        auto policy = thrust::cuda::par.on(stream);
        ...
        thrust::exclusive_scan(policy, ptr, ptr + a.parent_count + 1, ptr);
    }
    catch (const std::exception &exception)
    {
        std::fprintf(stderr, "Thrust error scanning candidate pairs: %s\n",
                     exception.what());
        cudaGetLastError();
        return false;
    }
    if (!detail::copy_scalar_to_host(
            ...
            &result.raw_pair_count, "read Super16 raw pair count"))
        return false;
    ...
    if (!detail::cuda_ok(cudaGetLastError(),
                         "launch Super16 row-work mapping"))
        return false;

    result.plan.owner_count = a.parent_row_count;
    if (!result.plan.owner_work.allocate(
            a.parent_row_count, "allocate Super16 candidate owner work"))
        return false;
    ...
    if (!finalize_work_plan(stream, &result.plan,
                            kCandidateWorkQuantum, error.get()) ||
        !result.candidate_row_ptr.allocate(
            static_cast<std::uint64_t>(a.parent_row_count) + 1,
            "allocate Super16 candidate row pointer") ||
        !detail::cuda_ok(cudaMemsetAsync(
                             result.candidate_row_ptr.get(), 0,
                             (static_cast<std::size_t>(a.parent_row_count) + 1) *
                                 sizeof(std::uint64_t),
                             stream),
                         "clear Super16 candidate row pointer"))
        return false;

    const std::uint64_t bitmap_words_per_row =
        b.parent_col_count == 0
            ? 0
            : 1 + (static_cast<std::uint64_t>(b.parent_col_count) - 1) / 32;
    if (bitmap_words_per_row >
        std::numeric_limits<std::size_t>::max() / sizeof(std::uint32_t))
    {
        std::fprintf(stderr, "Super16 candidate bitmap row is too wide.\n");
        return false;
    }
    const std::size_t bitmap_row_bytes =
        static_cast<std::size_t>(bitmap_words_per_row) * sizeof(std::uint32_t);
    std::uint32_t batch_row_capacity = 0;
    if (a.parent_row_count > 0)
    {
        const std::size_t capacity_wide =
            bitmap_row_bytes == 0
                ? static_cast<std::size_t>(a.parent_row_count)
                : std::max<std::size_t>(
                      1, kCandidateBitmapBudgetBytes / bitmap_row_bytes);
        batch_row_capacity = static_cast<std::uint32_t>(
            std::min<std::size_t>(capacity_wide, a.parent_row_count));
    }
    const std::uint64_t batch_count =
        batch_row_capacity == 0
            ? 0
            : 1 + (static_cast<std::uint64_t>(a.parent_row_count) - 1) /
                      batch_row_capacity;
    DeviceBuffer<std::uint32_t> bitmap;
    if (!bitmap.allocate(
            static_cast<std::uint64_t>(batch_row_capacity) *
                bitmap_words_per_row,
            "allocate bounded Super16 candidate bitmap"))
        return false;

    std::vector<detail::EventInterval> pass1_or_events;
    std::vector<detail::EventInterval> popcount_events;
    std::vector<detail::EventInterval> pass2_or_events;
    std::vector<detail::EventInterval> emit_events;
    if (metrics != nullptr)
    {
        pass1_or_events.reserve(static_cast<std::size_t>(batch_count));
        popcount_events.reserve(static_cast<std::size_t>(batch_count));
        pass2_or_events.reserve(static_cast<std::size_t>(batch_count));
        emit_events.reserve(static_cast<std::size_t>(batch_count));
    }
    const auto begin_timed_event =
        [&](std::vector<detail::EventInterval> *events) -> bool {
        if (metrics == nullptr)
            return true;
        events->emplace_back();
        return events->back().create() && events->back().record_begin(stream);
    };
    const auto end_timed_event =
        [&](std::vector<detail::EventInterval> *events) -> bool {
        return metrics == nullptr || events->back().record_end(stream);
    };

    const auto queue_begin = std::chrono::steady_clock::now();
    for (std::uint32_t row_begin = 0; row_begin < a.parent_row_count;
         row_begin += batch_row_capacity)
    {
        const std::uint32_t active_rows =
            std::min<std::uint32_t>(batch_row_capacity,
                                    a.parent_row_count - row_begin);
        const std::uint32_t row_end = row_begin + active_rows;
        const std::size_t active_bitmap_bytes =
            static_cast<std::size_t>(active_rows) * bitmap_row_bytes;
        if (!begin_timed_event(&pass1_or_events) ||
            (active_bitmap_bytes > 0 &&
             !detail::cuda_ok(cudaMemsetAsync(bitmap.get(), 0,
                                              active_bitmap_bytes, stream),
                              "clear Super16 pass1 bitmap")) ||
            !detail::cuda_ok(cudaMemsetAsync(result.plan.ticket.get(), 0,
                                             sizeof(unsigned long long), stream),
                             "reset Super16 pass1 ticket"))
            return false;
        if (bitmap_words_per_row > 0)
            detail::candidate_bitmap_or_queue_kernel
                <<<persistent_blocks(), kQueueThreads, 0, stream>>>(
                    a, b, result.a_parent_pair_prefix.get(),
                    result.row_work_ptr.get(),
                    result.plan.owner_item_ptr.get(), result.plan.items.get(),
                    row_begin, row_end, bitmap_words_per_row,
                    result.plan.ticket.get(), bitmap.get(), error.get());
        if (!detail::cuda_ok(cudaGetLastError(),
                             "launch Super16 candidate pass1 OR") ||
            !end_timed_event(&pass1_or_events) ||
            !begin_timed_event(&popcount_events))
            return false;
        if (bitmap_words_per_row > 0)
            detail::candidate_bitmap_popcount_kernel
                <<<active_rows, kBuildThreads, 0, stream>>>(
                    bitmap.get(), row_begin, active_rows,
                    bitmap_words_per_row, result.candidate_row_ptr.get());
        if (!detail::cuda_ok(cudaGetLastError(),
                             "launch Super16 candidate bitmap popcount") ||
            !end_timed_event(&popcount_events))
            return false;
    }
    if (!detail::sync_and_read_error(stream, error.get(),
                                     "Super16 candidate SPA pass1"))
        return false;
    const auto pass1_end = std::chrono::steady_clock::now();

    const auto scan_begin = pass1_end;
    try
    {
        auto policy = thrust::cuda::par.on(stream);
        thrust::device_ptr<std::uint64_t> ptr(
    ...
    {
        std::fprintf(stderr, "Thrust error scanning SPA candidate rows: %s\n",
                     exception.what());
        cudaGetLastError();
        return false;
    }
    if (!detail::copy_scalar_to_host(
            ...
            &result.candidate_count, "read Super16 SPA candidate count"))
        return false;
    ...
    {
        std::fprintf(stderr,
                     "Super16 candidate owners exceed the 32-bit WorkItem ABI.\n");
        return false;
    }
    if (!result.keys.allocate(result.candidate_count,
                              "allocate exact Super16 candidate keys"))
        return false;
         ...
         row_begin += batch_row_capacity)
    {
        ...
        const std::size_t active_bitmap_bytes =
            static_cast<std::size_t>(active_rows) * bitmap_row_bytes;
        if (!begin_timed_event(&pass2_or_events) ||
            (active_bitmap_bytes > 0 &&
             !detail::cuda_ok(cudaMemsetAsync(bitmap.get(), 0,
                                              active_bitmap_bytes, stream),
                              "clear Super16 pass2 bitmap")) ||
            !detail::cuda_ok(cudaMemsetAsync(result.plan.ticket.get(), 0,
                                             sizeof(unsigned long long), stream),
                             "reset Super16 pass2 ticket"))
            return false;
        if (bitmap_words_per_row > 0)
            detail::candidate_bitmap_or_queue_kernel
                <<<persistent_blocks(), kQueueThreads, 0, stream>>>(
                    ...
                    result.plan.ticket.get(), bitmap.get(), error.get());
        if (!detail::cuda_ok(cudaGetLastError(),
                             "launch Super16 candidate pass2 OR") ||
            !end_timed_event(&pass2_or_events) ||
            !begin_timed_event(&emit_events))
            return false;
        if (bitmap_words_per_row > 0)
            detail::candidate_bitmap_emit_kernel
                <<<active_rows, kBuildThreads, 0, stream>>>(
                    bitmap.get(), row_begin, active_rows,
                    bitmap_words_per_row, b.parent_col_count,
                    result.candidate_row_ptr.get(), result.keys.get(),
                    error.get());
        if (!detail::cuda_ok(cudaGetLastError(),
                             "launch Super16 candidate SPA emit") ||
            !end_timed_event(&emit_events))
            return false;
    }
    if (!detail::sync_and_read_error(stream, error.get(),
                                     "Super16 candidate SPA pass2"))
        return false;
    const auto pass2_end = std::chrono::steady_clock::now();

    if (metrics != nullptr)
    {
        const auto sum_events =
            [](const std::vector<detail::EventInterval> &events,
               double *total) -> bool {
            *total = 0.0;
            for (const detail::EventInterval &event : events)
            {
                double elapsed = 0.0;
        ...
        if (!sum_events(pass1_or_events,
                        &metrics->candidate_pass1_or_ms) ||
            !sum_events(popcount_events,
                        &metrics->candidate_popcount_ms) ||
            !sum_events(pass2_or_events,
                        &metrics->candidate_pass2_or_ms) ||
            !sum_events(emit_events, &metrics->candidate_emit_ms))
            return false;
        ...
        metrics->candidate_bitmap_batch_count = batch_count;
        metrics->candidate_bitmap_words_per_row = bitmap_words_per_row;
        metrics->candidate_bitmap_bytes = bitmap.bytes();
        metrics->candidate.owner_count = result.plan.owner_count;
        ...
        metrics->candidate.mean_owner_work =
            result.plan.owner_count == 0
                ? 0.0
                : static_cast<double>(result.raw_pair_count) /
                      result.plan.owner_count;
        metrics->work_index_ms +=
            std::chrono::duration<double, std::milli>(queue_begin -
                                                       function_begin)
                .count();
        metrics->candidate_scan_ms =
            std::chrono::duration<double, std::milli>(scan_end - scan_begin)
                .count();
        metrics->candidate_queue_ms =
            metrics->candidate_pass1_or_ms +
            metrics->candidate_popcount_ms +
            metrics->candidate_pass2_or_ms + metrics->candidate_emit_ms;
        metrics->candidate_keygen_ms = metrics->candidate_queue_ms;
        metrics->sort_unique_ms = 0.0;
        metrics->candidate_sort_unique_ms = 0.0;
        metrics->work_item_bytes += result.plan.items.bytes();
        const std::size_t candidate_temporary_bytes =
            result.a_parent_pair_prefix.bytes() +
            result.row_work_ptr.bytes() + result.plan.owner_work.bytes() +
            result.plan.owner_item_ptr.bytes() + result.plan.items.bytes() +
            result.plan.ticket.bytes() + result.keys.bytes() + bitmap.bytes();
        metrics->scratch_bytes += candidate_temporary_bytes;
        metrics->sort_temporary_bytes_estimate = 0;
        metrics->peak_temporary_bytes =
            std::max(metrics->peak_temporary_bytes,
                     candidate_temporary_bytes);
    }
    *output = std::move(result);
    return true;
}
                             ...
                             SymbolicMetrics *metrics = nullptr)
{
    ...
    if (a.role != OperandRole::A8x4 || b.role != OperandRole::B4x8 ||
        ...
        (b.parent_count > 0 && b.col_idx == nullptr))
    {
        std::fprintf(stderr, "Invalid Super16 page-SPA operands.\n");
        return false;
    }

    CandidateState result;
    DeviceBuffer<int> error;
    if (!error.allocate(1, "allocate Super16 page-SPA error") ||
        !detail::cuda_ok(cudaMemsetAsync(error.get(), 0, sizeof(int), stream),
                         "clear Super16 page-SPA error") ||
        !result.a_parent_pair_prefix.allocate(
            static_cast<std::uint64_t>(a.parent_count) + 1,
            "allocate Super16 A-parent pair prefix") ||
        !result.row_work_ptr.allocate(
    ...
    if (parent_blocks != 0)
        ...
        return false;
    if (!detail::cuda_ok(cudaGetLastError(),
                         "launch Super16 page pair degrees"))
        return false;
    try
    {
        auto policy = thrust::cuda::par.on(stream);
    ...
    {
        std::fprintf(stderr, "Thrust error scanning page candidate pairs: %s\n",
                     exception.what());
        cudaGetLastError();
        return false;
    }
    ...
    detail::map_row_work_kernel
        <<<detail::build_blocks(static_cast<std::uint64_t>(a.parent_row_count) + 1),
           kBuildThreads, 0, stream>>>(
            a, result.a_parent_pair_prefix.get(), result.row_work_ptr.get());
    if (!detail::cuda_ok(cudaGetLastError(),
                         "launch Super16 page row-work mapping"))
        return false;

    result.plan.owner_count = a.parent_row_count;
    if (!result.plan.owner_work.allocate(
            a.parent_row_count, "allocate Super16 page candidate owner work"))
        return false;
    if (a.parent_row_count > 0)
        detail::candidate_owner_work_kernel
            <<<detail::build_blocks(a.parent_row_count), kBuildThreads, 0,
    ...
    if (!finalize_work_plan(stream, &result.plan,
                            kCandidateWorkQuantum, error.get()))
        return false;
    const auto work_index_end = std::chrono::steady_clock::now();

    const std::uint64_t pages_per_row =
        b.parent_col_count == 0
            ? 0
            : 1 + (static_cast<std::uint64_t>(b.parent_col_count) - 1) /
                      kCandidatePageColumns;
    const std::uint64_t page_grid =
        static_cast<std::uint64_t>(a.parent_row_count) * pages_per_row;
    const std::uint64_t page_upper_bound =
        std::min(result.raw_pair_count, page_grid);
    const std::uint32_t maximum_hash_capacity =
        detail::floor_power_of_two(
            kCandidatePageHashBudgetBytes /
            (sizeof(unsigned long long) + sizeof(std::uint32_t)));
    std::uint32_t hash_capacity = 0;
    if (page_upper_bound > 0)
    {
        const std::uint64_t desired =
            page_upper_bound <= maximum_hash_capacity / 2
                ? std::max<std::uint64_t>(2, page_upper_bound * 2)
                : maximum_hash_capacity;
        hash_capacity = desired >= maximum_hash_capacity
                            ? maximum_hash_capacity
                            : detail::ceil_power_of_two(desired);
        if (hash_capacity == 0)
        {
            std::fprintf(stderr,
                         "Super16 touched-page hash capacity is invalid.\n");
            return false;
        }
    }

    DeviceBuffer<unsigned long long> hash_keys;
    DeviceBuffer<std::uint32_t> hash_values_offsets;
    DeviceBuffer<std::uint64_t> page_keys;
    DeviceBuffer<std::uint32_t> page_bitmaps;
    DeviceBuffer<std::uint64_t> page_row_ptr;
    DeviceBuffer<std::uint32_t> page_candidate_counts;
    DeviceBuffer<std::uint64_t> page_candidate_local_offsets;
    std::uint32_t page_count = 0;
    detail::EventInterval hash_event;
    detail::EventInterval bitmap_event;
    detail::EventInterval emit_event;
    if (metrics != nullptr &&
        (!hash_event.create() || !bitmap_event.create() ||
         !emit_event.create()))
        return false;
    ...
    if (hash_capacity > 0)
    {
        if (!hash_keys.allocate(hash_capacity,
                                "allocate Super16 touched-page hash keys") ||
            !hash_values_offsets.allocate(
                static_cast<std::uint64_t>(hash_capacity) + 1,
                "allocate Super16 touched-page hash offsets") ||
            !detail::cuda_ok(cudaMemsetAsync(
                                 hash_keys.get(), 0, hash_keys.bytes(), stream),
                             "clear Super16 touched-page hash") ||
            !detail::cuda_ok(cudaMemsetAsync(result.plan.ticket.get(), 0,
                                             sizeof(unsigned long long), stream),
                             "reset Super16 touched-page ticket") ||
            (metrics != nullptr && !hash_event.record_begin(stream)))
            return false;
        if (result.plan.item_count > 0)
            detail::candidate_page_hash_queue_kernel
                <<<persistent_blocks(), kQueueThreads, 0, stream>>>(
                    a, b, result.a_parent_pair_prefix.get(),
                    result.row_work_ptr.get(), result.plan.items.get(),
                    result.plan.item_count, result.plan.ticket.get(),
                    hash_keys.get(), hash_capacity, error.get());
        if (!detail::cuda_ok(cudaGetLastError(),
                             "launch Super16 touched-page hash queue") ||
            (metrics != nullptr && !hash_event.record_end(stream)))
            return false;
        int hash_error = 0;
        if (!detail::cuda_ok(cudaMemcpyAsync(
                                 &hash_error, error.get(), sizeof(hash_error),
                                 cudaMemcpyDeviceToHost, stream),
                             "read Super16 touched-page hash status") ||
            !detail::cuda_ok(cudaStreamSynchronize(stream),
                             "complete Super16 touched-page hash"))
            return false;
        if (hash_error != 0)
        {
            if (metrics != nullptr)
            {
                metrics->candidate_page_hash_overflow = hash_error == 63;
                metrics->candidate_touched_page_upper_bound =
                    page_upper_bound;
                metrics->candidate_page_hash_capacity = hash_capacity;
            }
            std::fprintf(stderr,
                         "Super16 touched-page hash failed (code=%d%s).\n",
                         hash_error,
                         hash_error == 63 ? ", capacity exhausted" : "");
            return false;
        }
        compact_begin = std::chrono::steady_clock::now();

        detail::mark_page_hash_slots_kernel
            <<<detail::build_blocks(hash_capacity), kBuildThreads, 0, stream>>>(
                hash_keys.get(), hash_capacity,
                hash_values_offsets.get());
        try
        {
            auto policy = thrust::cuda::par.on(stream);
            thrust::device_ptr<std::uint32_t> offsets(
                hash_values_offsets.get());
            thrust::exclusive_scan(policy, offsets,
                                   offsets + hash_capacity + 1, offsets);
        }
        catch (const std::exception &exception)
        {
            std::fprintf(stderr,
                         "Thrust error scanning touched-page hash: %s\n",
                         exception.what());
            cudaGetLastError();
            return false;
        }
        if (!detail::cuda_ok(cudaMemcpyAsync(
                                 &page_count,
                                 hash_values_offsets.get() + hash_capacity,
                                 sizeof(page_count), cudaMemcpyDeviceToHost,
                                 stream),
                             "read Super16 touched-page count") ||
            !detail::cuda_ok(cudaStreamSynchronize(stream),
                             "complete touched-page count"))
            return false;
        if (page_count >= hash_capacity)
        {
            if (metrics != nullptr)
            {
                metrics->candidate_page_hash_overflow = true;
                metrics->candidate_touched_page_upper_bound =
                    page_upper_bound;
                metrics->candidate_page_hash_capacity = hash_capacity;
                metrics->candidate_touched_pages = page_count;
            }
            std::fprintf(stderr,
                         "Super16 touched-page hash exhausted its fixed budget.\n");
            return false;
        }
        if (!page_keys.allocate(page_count,
                                "allocate compact Super16 page keys"))
            return false;
        if (page_count > 0)
            detail::compact_page_hash_keys_kernel
                <<<detail::build_blocks(hash_capacity), kBuildThreads, 0,
                   stream>>>(hash_keys.get(), hash_values_offsets.get(),
                              hash_capacity, page_keys.get());
        try
        {
            auto policy = thrust::cuda::par.on(stream);
            thrust::device_ptr<std::uint64_t> keys(page_keys.get());
            thrust::sort(policy, keys, keys + page_count);
        }
        catch (const std::exception &exception)
        {
            std::fprintf(stderr, "Thrust error sorting touched pages: %s\n",
                         exception.what());
            cudaGetLastError();
            return false;
        }
        if (!detail::cuda_ok(cudaMemsetAsync(
                                 hash_keys.get(), 0, hash_keys.bytes(), stream),
                             "clear Super16 page lookup hash"))
            return false;
        if (page_count > 0)
            detail::build_page_lookup_kernel
                <<<detail::build_blocks(page_count), kBuildThreads, 0, stream>>>(
                    page_keys.get(), page_count, hash_keys.get(),
                    hash_values_offsets.get(), hash_capacity, error.get());
    }
    if (!detail::sync_and_read_error(stream, error.get(),
                                     "build Super16 page lookup"))
        return false;
    const auto compact_end = std::chrono::steady_clock::now();

    if (!page_bitmaps.allocate(
            static_cast<std::uint64_t>(page_count) * kCandidatePageWords,
            "allocate Super16 touched-page bitmaps") ||
        (page_count > 0 &&
         !detail::cuda_ok(cudaMemsetAsync(
                              page_bitmaps.get(), 0, page_bitmaps.bytes(),
                              stream),
                          "clear Super16 touched-page bitmaps")) ||
        !detail::cuda_ok(cudaMemsetAsync(result.plan.ticket.get(), 0,
                                         sizeof(unsigned long long), stream),
                         "reset Super16 page bitmap ticket") ||
        (metrics != nullptr && !bitmap_event.record_begin(stream)))
        return false;
    if (result.plan.item_count > 0)
        detail::candidate_page_bitmap_queue_kernel
            <<<persistent_blocks(), kQueueThreads, 0, stream>>>(
                a, b, result.a_parent_pair_prefix.get(),
                result.row_work_ptr.get(), result.plan.items.get(),
                result.plan.item_count, result.plan.ticket.get(),
                hash_keys.get(), hash_values_offsets.get(), hash_capacity,
                page_count, page_bitmaps.get(), error.get());
    if (!detail::cuda_ok(cudaGetLastError(),
                         "launch Super16 page bitmap queue") ||
        (metrics != nullptr && !bitmap_event.record_end(stream)) ||
        !detail::sync_and_read_error(stream, error.get(),
                                     "Super16 page bitmap OR"))
        return false;

    const auto enumerate_begin = std::chrono::steady_clock::now();
    if (!page_row_ptr.allocate(
            static_cast<std::uint64_t>(a.parent_row_count) + 1,
            "allocate Super16 page row pointer") ||
        !page_candidate_counts.allocate(
            page_count, "allocate Super16 page candidate counts") ||
        !page_candidate_local_offsets.allocate(
            page_count, "allocate Super16 page candidate local offsets") ||
        !detail::cuda_ok(cudaMemsetAsync(
                             page_row_ptr.get(), 0,
                             (static_cast<std::size_t>(a.parent_row_count) + 1) *
                                 sizeof(std::uint64_t),
                             stream),
                         "clear Super16 page row counts"))
        return false;
    if (page_count > 0)
    {
        detail::count_page_rows_kernel
            <<<detail::build_blocks(page_count), kBuildThreads, 0, stream>>>(
                page_keys.get(), page_count, a.parent_row_count,
                page_row_ptr.get(), error.get());
        detail::popcount_page_bitmaps_kernel
            <<<page_count, kCandidatePageWords, 0, stream>>>(
                page_bitmaps.get(), page_count,
                page_candidate_counts.get());
    }
    try
    {
        auto policy = thrust::cuda::par.on(stream);
        thrust::device_ptr<std::uint64_t> page_rows(page_row_ptr.get());
        thrust::inclusive_scan(policy, page_rows,
                               page_rows + a.parent_row_count + 1,
                               page_rows);
    }
    catch (const std::exception &exception)
    {
        std::fprintf(stderr, "Thrust error scanning Super16 page rows: %s\n",
                     exception.what());
        cudaGetLastError();
        return false;
    }
    if (a.parent_row_count > 0)
        detail::page_row_candidate_offsets_kernel
            <<<detail::build_blocks(a.parent_row_count), kBuildThreads, 0,
               stream>>>(a.parent_row_count, page_row_ptr.get(),
                          page_candidate_counts.get(),
                          page_candidate_local_offsets.get(),
                          result.candidate_row_ptr.get());
    try
    {
        auto policy = thrust::cuda::par.on(stream);
        thrust::device_ptr<std::uint64_t> rows(
            result.candidate_row_ptr.get());
        thrust::exclusive_scan(policy, rows,
                               rows + a.parent_row_count + 1, rows);
    }
    catch (const std::exception &exception)
    {
        std::fprintf(stderr,
                     "Thrust error scanning page-SPA candidate rows: %s\n",
                     exception.what());
        cudaGetLastError();
        return false;
    }
    if (!detail::copy_scalar_to_host(
            stream, result.candidate_row_ptr.get() + a.parent_row_count,
            &result.candidate_count, "read page-SPA candidate count"))
        return false;
    if (result.candidate_count >
        static_cast<std::uint64_t>(std::numeric_limits<std::uint32_t>::max()))
    {
        std::fprintf(stderr,
                     "Super16 page-SPA candidates exceed the WorkItem ABI.\n");
        return false;
    }
    if (!result.keys.allocate(result.candidate_count,
                              "allocate Super16 page-SPA candidate keys") ||
        (metrics != nullptr && !emit_event.record_begin(stream)))
        return false;
    if (page_count > 0)
        detail::emit_page_candidates_kernel
            <<<page_count, kCandidatePageWords, 0, stream>>>(
                page_keys.get(), page_bitmaps.get(), page_count,
                b.parent_col_count, page_candidate_local_offsets.get(),
                result.candidate_row_ptr.get(), result.keys.get(),
                error.get());
    if (!detail::cuda_ok(cudaGetLastError(),
                         "launch Super16 page candidate emit") ||
        (metrics != nullptr && !emit_event.record_end(stream)) ||
        !detail::sync_and_read_error(stream, error.get(),
                                     "emit Super16 page candidates"))
        return false;
    ...
    if (metrics != nullptr)
    {
        ...
        double emit_ms = 0.0;
        if ((hash_capacity > 0 && !hash_event.elapsed(&hash_ms)) ||
            !bitmap_event.elapsed(&bitmap_ms) ||
            !emit_event.elapsed(&emit_ms))
            return false;
        metrics->parent_a = a.parent_count;
        metrics->parent_b = b.parent_count;
        metrics->raw_candidate_pairs = result.raw_pair_count;
        metrics->parent_c_candidates = result.candidate_count;
        metrics->candidate_touched_pages = page_count;
        metrics->candidate_touched_page_upper_bound = page_upper_bound;
        metrics->candidate_page_hash_capacity = hash_capacity;
        metrics->candidate_page_hash_bytes =
            hash_keys.bytes() + hash_values_offsets.bytes();
        metrics->candidate_page_bitmap_bytes = page_bitmaps.bytes();
        metrics->candidate.owner_count = result.plan.owner_count;
        metrics->candidate.item_count = result.plan.item_count;
        metrics->candidate.total_structural_work = result.raw_pair_count;
        metrics->candidate.mean_owner_work =
            result.plan.owner_count == 0
                ...
                : static_cast<double>(result.raw_pair_count) /
                      result.plan.owner_count;
        metrics->work_index_ms +=
            std::chrono::duration<double, std::milli>(work_index_end -
                                                       function_begin)
                .count();
        metrics->candidate_page_hash_ms = hash_ms;
        metrics->candidate_page_compact_sort_ms =
            std::chrono::duration<double, std::milli>(compact_end -
                                                       compact_begin)
                .count();
        metrics->candidate_page_bitmap_or_ms = bitmap_ms;
        metrics->candidate_page_enumerate_ms =
            std::chrono::duration<double, std::milli>(enumerate_end -
                                                       enumerate_begin)
                .count();
        metrics->candidate_emit_ms = emit_ms;
        metrics->candidate_queue_ms = hash_ms + bitmap_ms;
        metrics->candidate_keygen_ms = metrics->candidate_queue_ms;
        metrics->sort_unique_ms =
            metrics->candidate_page_compact_sort_ms;
        metrics->candidate_sort_unique_ms = metrics->sort_unique_ms;
        metrics->work_item_bytes += result.plan.items.bytes();
        const std::size_t candidate_temporary_bytes =
            result.a_parent_pair_prefix.bytes() + result.row_work_ptr.bytes() +
            result.plan.owner_work.bytes() +
            result.plan.owner_item_ptr.bytes() + result.plan.items.bytes() +
            result.plan.ticket.bytes() + hash_keys.bytes() +
            hash_values_offsets.bytes() + page_keys.bytes() +
            page_bitmaps.bytes() + page_row_ptr.bytes() +
            page_candidate_counts.bytes() +
            page_candidate_local_offsets.bytes() +
            result.candidate_row_ptr.bytes() + result.keys.bytes();
        metrics->scratch_bytes += candidate_temporary_bytes;
        metrics->sort_temporary_bytes_estimate = page_keys.bytes();
        metrics->peak_temporary_bytes = std::max(
            metrics->peak_temporary_bytes,
            candidate_temporary_bytes + page_keys.bytes());
    }
    *output = std::move(result);
    return true;
#endif
