...
//    real aval, bval;
    ...
    // auto g = cg::tiled_partition<PWARP>(cg::this_thread_block());
    ...
    // g.sync();
// __syncwarp();
    for (j = d_arpt[rid]; j < d_arpt[rid + 1]; j++) {//__syncwarp();
        acol = ld_gbl_int32(d_acol + j);//__syncwarp();
//        aval = ld_gbl_real(d_aval + j);
        // g.sync();
// __syncwarp();
        for (k = d_brpt[acol] + tid; k < d_brpt[acol + 1]; k += PWARP) {//__syncwarp();
            // g.sync();
// __syncwarp();
...
//            bval = d_bval[k];
            ...
            // if(rid == 9){
            //     printf("tid: %d, bcol: %d, hash_pos: %d\n", tid, bcol, (bcol * HASH_SCAL) & ((B_PWMIN) - 1));
            // }
...
// //                        atomic_fadd(shared_value + adr, aval * bval);
//                         if(rid == 9){
//                             printf("hash insert: tid: %d, bcol: %d, hash_pos: %d, now_hash: %d, shared_check[%d]: %d\n", tid, bcol, (bcol * HASH_SCAL) & ((B_PWMIN) - 1), hash, adr, shared_check[adr]);
//                         }

//                         if(rid == 9 && tid == 0){
//                             printf("\n");
//                             for (int temp = 0; temp < B_PWMIN; temp++){
//                                 printf("inter_shared_check[%d] = %d, ", soffset + temp, shared_check[soffset + temp]);
//                             }
//                             printf("\n");
//                         }
                ...
                // if((loop_time % 2000 == 0) && (loop_time < 10000) && rid == 9 && tid == 0){
                //     printf("\ntrdIdx:%d, blkIdx: %d, Loop_time: %d, j: %d, k: %d\n", threadIdx.x, blockIdx.x, loop_time, j, k);
                //     loop_time++;
                //     for (int temp = 0; temp < B_PWMIN; temp++){
                //         printf("loop_shared_check[%d] = %d, ", soffset + temp, shared_check[soffset + temp]);
                //     }
                //     printf("\n\n");
                // }
...
// __syncwarp();
                ...
                // if (first_hash == hash && rid == 9 && tid == 0) loop_time++;

                // // if (first_hash == hash && loop_time == 10000 && rid == 124 && tid == 1){
                // if (first_hash == hash && loop_time == 10000 && rid == 9 && tid == 0){
                //     printf("rid: %d, tid: %d, rptr_a_size: %d, rptr_b_size: %d, loop_time: %d, j: %d, k: %d b_col: %d\n", rid, tid, d_arpt[rid + 1] - d_arpt[rid], d_brpt[acol + 1] - d_brpt[acol], loop_time, j, k, key);
                //     for (int temp = 0; temp < B_PWMIN; temp++){
                //         printf("relative_shared_check[%d] = %d, ", temp, shared_check[soffset + temp]);
                //         // if (shared_check[soffset + temp] == -1){
                //         //     printf("shared_check[%d] = %d, finds empty\n", temp, shared_check[soffset + temp]);
                //         //     break;
                //         // }
                //         // if(temp == B_PWMIN - 1) printf("shared_check full!\n");
                //     }
                //     printf("\n\n");
                //     for (int temp = 0; temp < B_PWMIN; temp++){
                //         printf("absolute_shared_check[%d] = %d, ", soffset + temp, shared_check[soffset + temp]);
                //     }
                //     // break;
                // }
//__syncwarp();
